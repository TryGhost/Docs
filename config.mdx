...
* **`useLibrary`** (boolean): Set to `true` to enable the advanced file rotation library (using a rotating file stream stream instead of classical time-only Bunyan rotation). Note that configuring any of `threshold`, `gzip`, or `rotateExisting` automatically defaults `useLibrary` to `true`.
...
* **`rotateExisting`** (boolean): Set to `true` to force a log rotation when Ghost starts up if a log file already exists.

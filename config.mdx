...
<Warning>
Some hosting providers block outbound SMTP. DigitalOcean blocks ports 25, 465 and 587 on all Droplets by default, and Google Cloud blocks port 25 on Compute Engine instances. On those providers, transactional email fails with a connection timeout until you set an alternative port — see [DigitalOcean](/faq/digitalocean-smtp-blocked/) and [Google Cloud](/faq/mail-config-error-google-cloud/).
</Warning>

...
<Warning>
Some hosting providers block outbound SMTP. DigitalOcean blocks ports 25, 465 and 587 on all Droplets by default, and Google Cloud blocks port 25 on Compute Engine instances. If the port your mail config resolves to is one of the blocked ones, transactional email fails with a connection timeout until you switch to a port your provider allows — see [DigitalOcean](/faq/digitalocean-smtp-blocked/) and [Google Cloud](/faq/mail-config-error-google-cloud/).
</Warning>
...
In addition to default time-based rotation, Ghost supports size-based log rotation and compression settings under the `rotation` key:

* **`useLibrary`** (boolean): Set to `true` to enable the advanced file rotation library (using a rotating file stream instead of classical time-only Bunyan rotation). Note that configuring any of `threshold`, `gzip`, or `rotateExisting` automatically defaults `useLibrary` to `true`.
* **`threshold`** (string): Limit the size of a log file before rotating it, e.g., `"10m"` (10 Megabytes) or `"500k"`.
* **`totalSize`** (string): Limit the total size cap of all rotated log files, e.g., `"100m"` or `"1g"`.
* **`gzip`** (boolean): Set to `true` to compress rotated log files using gzip.
* **`rotateExisting`** (boolean): Set to `true` to rotate an existing log file on startup when it would have rotated according to the configured period.

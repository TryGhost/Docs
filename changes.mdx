...
* Card assets will now be included by default, including bookmark and gallery cards. ([docs](/themes/helpers/config/))
...
* Tier benefits are now returned as a list of strings. ([docs](/themes/helpers/tiers/#fetching-tiers-with-the-get-helper))
* Paid Tiers now have numeric `monthly_price` and `yearly_price` attributes, and a separate `currency` attribute. ([docs](/themes/helpers/tiers/))

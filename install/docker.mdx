...
1. [A new install](https://docs.ghost.org/install/docker#install-ghost)
2. [Migrating an existing Ghost-CLI site](https://docs.ghost.org/install/docker#migrating-a-ghost-cli-install)
...
3. If you're using a www domain for `DOMAIN` in `.env`:
   Redirect your root domain to the www variant by setting up DNS for both domains, then uncomment the "Redirect root -\> www domain" block" and set `CHANGE_ME` to your root domain without the [www](http://www). This is required for ActivityPub to work correctly with www domains.

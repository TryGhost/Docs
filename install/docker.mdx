...
2. To redirect the www variant to your root domain, setup DNS for both, ensure that `DOMAIN` in `.env` is set to the root domain and then uncomment the "Redirect www -> root domain" block.
3. To redirect your root domain to the www variant, setup DNS for both, ensure that `DOMAIN` in `.env` is set to the www domain, and then uncomment the "Redirect root -> www domain" block" and set `CHANGE_ME` to your root domain without the www.
...
Change the `COMPOSE_PROFILES` variable to include activitypub:

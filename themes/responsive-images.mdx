...
Once image sizes are defined, pass a `size` parameter to the [\{\{img\_url}}](/themes/helpers/img_url/) helper in your theme to output an image at a particular size.
...
Pass a `format` parameter to the [\{\{img\_url}}](/themes/helpers/img_url/) helper in your theme to output an image in a particular image format. This only works in combination with the `size` parameter.

...
`{{plural}}` is a [formatting helper](/themes/helpers/plural/) for outputting strings which change depending on whether a number is singular or plural.
...
The [reading time helper](/themes/helpers/reading_time/) can be used in translations to provide a reading time for your posts in the desired language.
...
The [`{{meta_title}}`](/themes/helpers/meta_data/) helper accepts a page parameter that can be used in conjunction with translations. By using the follow it’s possible to translate the word “Page” shown in the title of paginated pages:

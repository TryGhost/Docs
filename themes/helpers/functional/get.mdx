...
Fetch all tags and output them using the [tags helper](/themes/helpers/tags).
...
The `{{#get}}` helper supports two parameters. The first entry refers to your returned data collection. The second entry refers to your [pagination object](/themes/helpers/pagination/).
...
The `primary_tag` represents the first tag on a post. See the available [attributes](/themes/helpers/tags/#list-of-attributes).

...
The most common gotcha when fetching posts from the Content API is not using the [include](#include) parameter to request related data such as tags and authors. By default, the response for a post will not include these:
...
Tags are the [primary taxonomy](/docs/publishing/#tags) within a Ghost site.
...
By default, internal tags are always included, use `filter=visibility:public` to limit the response directly or use the [tags helper](/docs/themes/helpers/tags/) to handle filtering and outputting the response.
...
Authors are a subset of [users](/docs/staff/) who have published posts associated with them.
...
The values provided as query parameters MUST be url encoded when used directly. The [client libraries](/docs/content-api/javascript/) will handle this for you.
...
Filter strings must be URL encoded. The [\{\{get}}](/docs/themes/helpers/get/) helper and [client library](/docs/content-api/javascript/) handle this for you.

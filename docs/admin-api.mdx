...
We’ve developed an [API client for JavaScript](/docs/admin-api/javascript/), that simplifies authenticating with the admin API, and makes reading and writing data a breeze. The client is designed for use with integrations, supporting token authentication and the endpoints available to integrations.
...
Use the `Accept-Version` header to indicate the minimum version of Ghost’s API to operate with. See [API Versioning](/docs/faq/api-versioning/) for more details.
...
* `meta`: contains [pagination](/docs/content-api/#pagination) information for browse requests.
...
The values provided as query parameters MUST be url encoded when used directly. The [client library](/docs/admin-api/javascript/) will handle this for you.

For more details see the [Content API](/docs/content-api/#parameters).
...
See the [Content API](/docs/content-api/#errors).
...
These are the endpoints & methods currently available to integrations. More endpoints are available through user authentication. Each endpoint has a stability index, see [versioning](/docs/faq/api-versioning) for more information.
...
| [/newsletters/](/docs/admin-api/#newsletters) | Browse, Read, Edit, Add               | Stable    |
...
Posts are the [primary resource](/docs/publishing/) in a Ghost site, providing means for publishing, managing and displaying content.
...
Pages are [static resources](/docs/publishing/) that are not included in channels or collections on the Ghost front-end. They are identical to posts in terms of request and response structure when working with the APIs.
...
Below is a minimal example to create a webhook using [token authenticated](#token-authentication) request:
